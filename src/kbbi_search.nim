...
proc printSense(
    sense: seq[string], examples: seq[string], xrefs: seq[string], indent = "  "
) =
  if sense.len < 13:
    return
  ...
  if sense[6].len > 0:
    parts.add(sense[6])
  if sense[7].len > 0:
    parts.add("(bentuk tidak baku: " & sense[7] & ")")
  if sense[8].len > 0:
    parts.add("(lihat: " & sense[8] & ")")
  if sense[9].len > 0:
    parts.add("; " & sense[9])
  if sense[11].len > 0:
    parts.add("= " & sense[11])
...
proc printEntry(db: DbConn, id: int, nilai, word, kind: string) =
  ...
  for row in db.fastRows(
    ...
    for exRow in db.fastRows(
      sql"""
      SELECT example FROM sense_examples WHERE sense_id = ? ORDER BY id""",
      row[0],
    ):
      examples.add(exRow[0])

    var xrefs: seq[string]
    for xRow in db.fastRows(
      sql"""
      SELECT xref_id FROM sense_xrefs WHERE sense_id = ?""",
      row[0],
    ):
      xrefs.add(xRow[0])
...
proc printShort(pos, text, example: string, id, word: string) =
  ...
  if preview.len > previewLen:
    preview = preview[0 ..< previewLen] & "..."
...
proc main() =
  ...
  of "--id":
    ...
    let row = db.getRow(
      sql"""
      SELECT id, nilai, word, kind FROM entries WHERE id = ?""",
      paramStr(3),
    )
    if row[0] == "":
      echo "Not found"
  ...
  of "--exact":
    ...
    var row = db.getRow(
      sql"""
      SELECT id, nilai, word, kind FROM entries WHERE nilai = ?""",
      norm,
    )
    if row[0] == "":
      row = db.getRow(
        sql"""
        SELECT id, nilai, word, kind FROM entries WHERE nilai_norm = ?""",
        fnorm,
      )
    if row[0] == "":
      echo "'" & paramStr(3) & "' not found"
    else:
      printEntry(db, parseInt(row[0]), row[1], row[2], row[3])
  of "--kat":
    if paramCount() < 4:
    ...
    for row in db.fastRows(
      ...
      WHERE """ &
          likeClause & " ORDER BY e.nilai LIMIT " & $categoryLimit
      ),
      nilai,
    ):
    ...
    let total = db.getValue(
      sql(
        "SELECT COUNT(DISTINCT e.id) FROM entries e " &
          "JOIN senses s ON e.id = s.entry_id " & "WHERE " & likeClause
      ),
      nilai,
    )
    echo "\n" & total & " total for " & jenis & "=" & nilai & " (showing " & $count & ")"
  of "--fts":
    if paramCount() < 3:
      printUsage()
      quit(1)
  ...
  of "--list":
    ...
    for row in db.fastRows(listQ):
      echo "  " & alignLeft(row[0], 30) & " " & alignLeft(row[1], 40) & " (" & row[2] &
        ")"
  else:
