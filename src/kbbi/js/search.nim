...
proc searchFTS*(query: cstring): SearchResult =
  ...
  let ftsSql: cstring =
    ...
    cstring($resultLimitFts)
  {.
    emit: [
      """
    try {
      """, res, """ = """, sqlDb, """.exec(""", ftsSql, """, [""",
      safeQuery,
      """]);
    } catch(e) { """, ftsErr,
      """ = String(e.message || e); }
  ...
  let html =
    buildResultHeader("FTS —", cstring($rows.len & " hasil")) & buildResultCards(rows)
...
proc searchKat*(jenis, nilai, query: cstring): SearchResult =
  ...
  if hasQuery:
    {.
      emit: [
        res, " = ", sqlDb, ".exec(", likeQ, ", [", nilai, ",", norm, ",", nextNorm, ",",
        fnorm, ",", nextFnorm, "]);",
      ]
    .}
  else:
  ...
  if hasQuery:
    {.
      emit: [
        countRes, " = ", sqlDb, ".exec(", countQ, ", [", nilai, ",", norm, ",",
        nextNorm, ",", fnorm, ",", nextFnorm, "]);",
      ]
    .}
  else:
  ...
  var listHtml: cstring =
    ...
    " kata ditemukan" &
    (if ($total).parseInt != rows.len: ", menampilkan " & cstring($rows.len)
    else: "") & "</p>"
  return SearchResult(html: listHtml, found: true, word: "", error: SearchOk)
